...
use crate::{error::CLIError, types::ActionValuator};

pub async fn stock_buy(av: ActionValuator) -> Result<Order, CLIError> {
...
pub async fn stock_sell(av: ActionValuator) -> Result<Order, CLIError> {
    let amount = (av.strength * 10.0) as i64;
    let api_info = ApiInfo::from_env().unwrap();
    let client = Client::new(api_info);
    let request = order::CreateReqInit {
